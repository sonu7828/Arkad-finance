...
export default function AgentLayout() {
  ...
  const { user } = useAuth();
  const { branding } = useSidebarRoleConfig('agent');
  ...
  return (
    <div className="flex h-screen bg-slate-50/50 overflow-hidden font-sans">
      ...
      <aside className={`fixed inset-y-0 left-0 w-[280px] z-[60] lg:hidden transform transition-transform duration-300 ease-in-out ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <RoleSidebarNav routeRole="agent" isCollapsed={false} setIsSidebarCollapsed={setIsSidebarCollapsed} onLinkClick={() => setIsSidebarOpen(false)} onClose={() => setIsSidebarOpen(false)} />
      </aside>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <header className="h-16 flex items-center justify-between px-4 md:px-8 bg-white/80 backdrop-blur-md border-b border-slate-200/60 sticky top-0 z-40 transition-shadow duration-300">
          ...
          <div className="flex items-center gap-4">
            <Link to="/agent/notifications" className="hidden sm:flex w-9 h-9 items-center justify-center rounded-lg bg-white border border-slate-200 text-slate-400 hover:text-primary hover:border-primary/20 transition-all relative">
              <Bell size={18} />
              <span className="absolute top-2 right-2 w-2 h-2 bg-rose-500 rounded-full border-2 border-white" />
            </Link>

            <Link to={branding.profilePath || '/agent/profile'} className="flex items-center gap-3 pl-2 pr-1 py-1 rounded-xl hover:bg-slate-50 transition-all group border-none">
              <div className="text-right hidden md:block leading-tight">
                <p className="text-sm font-bold text-slate-800 leading-none">{user?.name?.split(' ')[0] || 'Agent'}</p>
                <p className="text-[10px] font-bold text-slate-400 mt-0.5 uppercase tracking-widest leading-none">Partnership Agent</p>
              </div>

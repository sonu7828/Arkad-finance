...
export default function AdminLayout() {
  ...
  const { user } = useAuth();
  const { branding } = useSidebarRoleConfig('admin');
  ...
  return (
    <div className="flex h-screen bg-slate-50/50 overflow-hidden font-sans">
      ...
      <aside className={`fixed inset-y-0 left-0 w-[280px] z-[60] lg:hidden transform transition-transform duration-300 ease-in-out ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <RoleSidebarNav routeRole="admin" isCollapsed={false} setIsSidebarCollapsed={setIsSidebarCollapsed} onLinkClick={() => setIsSidebarOpen(false)} onClose={() => setIsSidebarOpen(false)} />
      </aside>
        ...
        <header className="h-16 flex items-center justify-between px-4 md:px-8 bg-white/80 backdrop-blur-md border-b border-slate-200/60 sticky top-0 z-40 transition-shadow duration-300">
          <div className="flex items-center gap-4">
            ...
            <div className="hidden lg:flex items-center gap-2 relative group w-64 xl:w-80">
              ...
              <input type="text" placeholder="Search data, logs, borrowers..." className="w-full pl-11 pr-4 py-2 bg-slate-100/50 border border-transparent rounded-xl text-sm font-medium focus:ring-4 focus:ring-primary/5 focus:bg-white focus:border-primary/20 transition-all outline-none placeholder:text-slate-400" />
            </div>
          ...
          <div className="flex items-center gap-2 md:gap-4">
            ...
            <Link to={branding.profilePath || '/admin/profile'} className="flex items-center gap-3 pl-2 pr-1 py-1 rounded-xl hover:bg-slate-50 transition-all group border-none">
              <div className="text-right hidden md:block leading-tight">
                <p className="text-sm font-bold text-slate-800 leading-none">{user?.name?.split(' ')[0] || 'Admin'}</p>
                <p className="text-[10px] font-bold text-slate-400 mt-0.5 uppercase tracking-widest leading-none">Administrator</p>
              </div>
